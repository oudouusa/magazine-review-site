...
import { type MhIssueDetail, type MhMagazine } from "@/lib/magazine-hub-db";
import {
  getAdjacentCards,
  getCardPerformers,
  getIssueDetailCached,
  getMagazineCardDetailCached,
} from "@/lib/mh-insights";
import { safeHttpUrl, cssBgUrl } from "@/lib/safe-url";
...
function getMagazineDetail(ref: MagazineRef) {
  return ref.kind === "card" ? getMagazineCardDetailCached(ref.id) : getIssueDetailCached(ref.id);
}
...
function amazonHref(issue: MhIssueDetail): string {
  return safeHttpUrl(issue.amazonUrl) ?? `https://www.amazon.co.jp/s?k=${purchaseQuery(issue)}&tag=magazinelab-22`;
}

function rakutenHref(issue: MhIssueDetail): string {
  return safeHttpUrl(issue.rakutenUrl) ?? `https://search.books.rakuten.co.jp/bks/genesis/search/?g=001&sitem=${purchaseQuery(issue)}`;
}
...
function AdjacentLink({ label, item }: { label: string; item: MhMagazine }) {
  return (
    <Link href={`/magazines/${item.slug}`} className="mag-adjacent-link mh-lift">
      <span>{label}</span>
      <strong>{item.seriesName} {item.issue}</strong>
      <time className="mh-num">{item.releaseDate}</time>
    </Link>
  );
}
...
export default async function MagazineDetailPage({ params }: Props) {
  ...
  const coverImageUrl = safeHttpUrl(issue.coverImageUrl);
  const quickFacts = [
  ...
  return (
    <>
            ...
            <div
              ...
              style={{
                background: coverImageUrl
                  ? `${cssBgUrl(coverImageUrl)} center / cover no-repeat, linear-gradient(160deg, ${issue.gradient.c1}, ${issue.gradient.c2})`
          ...
          <div className="mag-meta">
            ...
            <div className="mag-facts">
              {quickFacts.map(({ label, value }) => (
                <div key={label}>
                  <span>{label}</span>
                  <strong className="mh-num">{value}</strong>
                </div>
              ))}
            </div>

            {sourceUrl && (
              <a href={sourceUrl} target="_blank" rel="nofollow noopener" className="btn btn-ghost mag-source-link">
                記事を見る
              </a>
            )}
          </div>
        ...
        {performerCount > 0 && (
          <section className="mag-detail-section">
            <div className="section-head">
              ...
              <span className="mag-section-count mh-num">{performerCount}名</span>
            </div>
          ...
          <section className="mag-detail-section">
            ...
            <div className="mh-rail">
              {issue.backnumbers.map((bn) => {
                const bnCover = safeHttpUrl(bn.coverImageUrl);
                return (
                  <Link key={bn.slug} href={`/magazines/${bn.slug}`} className="mag-backnumber-card mh-lift">
                    <span
                      style={{
                        background: bnCover
                          ? `linear-gradient(180deg, rgba(0,0,0,0) 50%, rgba(0,0,0,.72) 100%), ${cssBgUrl(bnCover)} center top / cover no-repeat`
                          : `linear-gradient(160deg, ${bn.gradient.c1}, ${bn.gradient.c2})`,
                      }}
                    >
                      <em>{bn.issue}</em>
                    </span>
                    <time className="mh-num">{bn.releaseDate}</time>
                  </Link>
                );
              })}
            </div>
      ...
      <style>{`
        ...
        .mag-detail-breadcrumb a { color: var(--ink-3); text-decoration: none; }
        .mag-detail-page {
          max-width: 1200px;
          margin: 0 auto;
        ...
        .mag-cover strong, .mag-cover span {
          font-family: "Noto Serif JP", serif;
          font-weight: 900;
          line-height: 1.35;
          text-shadow: 0 2px 14px rgba(0,0,0,.32);
        }
        .mag-cover strong { font-size: 24px; }
        .mag-cover span { font-size: 14px; }
        .mag-meta {
          min-width: 0;
          border: 1px solid var(--line);
          border-radius: 10px;
        ...
        .mag-facts span { display: block; color: var(--ink-3); font-size: 10px; margin-bottom: 4px; letter-spacing: .1em; }
        .mag-facts strong { display: block; color: var(--ink); font-size: 13px; line-height: 1.35; }
        .mag-source-link { margin-top: 18px; }
        .mag-buy-card {
          display: grid;
          gap: 10px;
          border: 1px solid color-mix(in srgb, var(--amber) 34%, transparent);
          border-radius: 10px;
          background: linear-gradient(180deg, color-mix(in srgb, var(--amber) 10%, transparent), var(--paper) 42%);
          padding: 18px;
          box-shadow: var(--shadow-lift);
        }
        ...
        .mag-buy-card .btn { justify-content: center; width: 100%; }
        .mag-buy-card p { margin: 4px 0 0; color: var(--ink-3); font-size: 11px; line-height: 1.55; }
        .mag-detail-section { margin-top: var(--row-gap); }
        .mag-section-count { flex-shrink: 0; color: var(--ink-3); font-size: 11px; padding-bottom: 6px; }
        .mag-performer-chips { display: flex; flex-wrap: wrap; gap: 10px; }
        .mag-performer-chip {
          display: inline-flex;
          min-height: 36px;
          align-items: center;
        ...
        .mag-performer-chip:not(.plain):hover { border-color: var(--primary); color: var(--primary); }
        .mag-performer-chip.plain { color: var(--ink-2); }
        .mag-backnumber-card {
          flex: 0 0 112px;
          color: inherit;
          text-align: center;
        ...
        .mag-backnumber-card time { display: block; margin-top: 6px; color: var(--ink-3); font-size: 10px; }
        .mag-adjacent {
          display: grid;
          grid-template-columns: repeat(2, minmax(0, 1fr));
          gap: var(--gap);
        ...
        .mag-adjacent-link time { display: block; margin-top: 4px; color: var(--ink-3); font-size: 11px; }
        @media (max-width: 980px) {
          .mag-hero { grid-template-columns: minmax(220px, 320px) minmax(0, 1fr); }
          .mag-buy-card { grid-column: 1 / -1; }
        }
        @media (max-width: 700px) {
          .mag-detail-page { padding-left: 12px; padding-right: 12px; }
          .mag-hero { grid-template-columns: 1fr; }
          .mag-cover-wrap { max-width: 340px; }
          .mag-facts, .mag-adjacent { grid-template-columns: 1fr; }
          .mag-adjacent > span { display: none; }
        }
      `}</style>
    </>
  );
